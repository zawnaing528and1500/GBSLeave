...
namespace GBSLeave.App_Code
{
    public class Leave
    {
        ...
        //
        public DataTable GetHandOverEmployee(int branch, int dept, int empID)
        {
            string constr = ConfigurationManager.ConnectionStrings["constr"].ConnectionString;
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("SELECT EmpName FROM dbo.Employee WHERE BranchID=" + branch + "AND DeptID=" + dept + "AND ID !=" + empID))
                {
        ...
        public DataTable getAllLeave()
        {
            ...
            using (SqlConnection con = new SqlConnection(constr))
            {
                using (SqlCommand cmd = new SqlCommand("select * from Leave"))
                {
                    using (SqlDataAdapter sda = new SqlDataAdapter())
                    {
                        cmd.Connection = con;
